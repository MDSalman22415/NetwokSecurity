...
import os
import sys
from networksecurity.utils.main_utils.utils import read_yaml_file, write_ymal_file
...
class DataValidation:
    ...
    def validate_number_of_numerical_columns(
            self, dataframe: pd.DataFrame) -> bool:
        try:
            number_of_numerical_columns = len(
                self.__schema_config["numerical_columns"]
            )

            dataframe_numerical_columns = len(
                dataframe.select_dtypes(include=["number"]).columns
            )

            logging.info(
                f"Required number of numerical columns :"
                f"{number_of_numerical_columns}"
            )

            logging.info(
                f"Data has numerical columns :"
                f"{dataframe_numerical_columns}"
            )

            if dataframe_numerical_columns == number_of_numerical_columns:
                return True
    ...
    def initaite_data_validation(self) -> DataValidationArtifact:
        try:
            ...
            status = self.validate_number_of_numerical_columns(
                dataframe=test_dataframe
            )
            ...
            status = self.detact_dataset_drift(
